...
function sqrlProtocol(options) {
  ...
  const hasSameHmac = (what, hmac) =>
    signHmac(what.toString(), opts.hmacSecret) === hmac;
  ...
  const createNut = async (ip, userId, question) => {
    debug('Create nut %s, %d, %o', ip, userId, question);
    const ask = [
